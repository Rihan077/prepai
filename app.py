...
from rag_pipeline import (
    ...
    validate_pdf, build_pdf_index, multi_pdf_retrieve,
    BM25_AVAILABLE, RAGAS_AVAILABLE, NX_AVAILABLE,
    MAX_PDFS, MAX_PDF_SIZE_MB
)
...
.eval-card {
    ...
    padding: 16px 20px; margin: 12px 0; box-shadow: 0 2px 8px #0f204008;
}
...
.chat-user {
    ...
    border-radius: 14px 14px 4px 14px; padding: 12px 16px; margin: 8px 0; color: #0f2040;
}
.chat-ai {
    background: #ffffff; border: 1.5px solid #e2e8f0;
    border-left: 4px solid #f59e0b; border-radius: 14px 14px 14px 4px;
    padding: 12px 16px; margin: 8px 0; color: #0f2040; box-shadow: 0 2px 8px #0f204008;
}
...
.quiz-card {
    background: #ffffff; border: 1.5px solid #e2e8f0;
    border-radius: 12px; padding: 20px; margin: 10px 0; box-shadow: 0 2px 8px #0f204008;
}
...
.topic-badge {
    ...
    font-weight: 600; margin: 3px 3px 3px 0; border: 1px solid #fde68a;
}
.system-status {
    background: #0f2040; border: 1px solid #1e3a6e;
...
.pdf-card {
    ...
    padding: 12px 16px; margin: 6px 0; box-shadow: 0 1px 4px #0f204008;
}
.pdf-name { color: #0a1628; font-size: 13px; font-weight: 600; }
.pdf-meta { color: #94a3b8; font-size: 11px; margin-top: 2px; }
 
.source-card {
    background: #faf5ff; border: 1.5px solid #ede9fe;
    border-radius: 10px; padding: 10px 14px; margin: 4px 0;
}
.source-pdf { color: #5b21b6; font-size: 12px; font-weight: 700; }
.source-pages { color: #7c3aed; font-size: 11px; }
 
.day-card {
    background: #ffffff; border: 1.5px solid #e2e8f0;
    border-top: 3px solid #f59e0b; border-radius: 14px;
    padding: 20px; margin: 10px 0; box-shadow: 0 2px 8px #0f204008;
}
.day-card-done {
    background: #f0fdf4; border: 1.5px solid #bbf7d0;
    border-top: 3px solid #059669; border-radius: 14px; padding: 20px; margin: 10px 0;
}
...
.day-topic-chip {
    ...
    padding: 3px 10px; font-size: 12px; margin: 3px; font-weight: 600;
}
.day-focus {
    color: #334155; font-size: 13px; margin: 10px 0;
...
.prepai-header {
    background: #0a1628; border-radius: 16px;
...
.feature-card {
    background: #ffffff; border: 1.5px solid #e2e8f0;
    border-radius: 12px; padding: 20px; box-shadow: 0 2px 8px #0f204008;
}
...
.doc-intel-card {
    background: #ffffff; border: 1.5px solid #e2e8f0;
    border-left: 4px solid #0a1628; border-radius: 12px;
    padding: 20px; margin: 12px 0; box-shadow: 0 2px 8px #0f204008;
}
...
.flashcard-front { background: #0a1628; border-radius: 14px 14px 0 0; padding: 24px; text-align: center; min-height: 90px; display: flex; align-items: center; justify-content: center; }
.flashcard-back { background: #ffffff; border: 1.5px solid #e2e8f0; border-top: none; border-radius: 0 0 14px 14px; padding: 16px 20px; min-height: 90px; }
...
.exam-timer { background: #0a1628; border-radius: 12px; padding: 16px 24px; text-align: center; margin-bottom: 16px; }
...
.short-answer-card { background: #ffffff; border: 1.5px solid #e2e8f0; border-top: 3px solid #0a1628; border-radius: 12px; padding: 20px; margin: 12px 0; }
...
div[data-testid="stButton"] button:hover { background: #f59e0b; color: #0a1628; }
</style>
...
def init_state():
    defaults = {
        "user": None,
        # Multi-PDF storage
        "pdf_indexes": [],      # list of index dicts
        "active_pdf_names": [], # list of filenames
        # Single PDF fallback
        "db": None, "bm25_index": None, "all_docs": None, "split_docs": None,
        "pdf_name": None,
        # Chat
        "chat": [], "memory": None,
        # Quiz
        ...
        # Study plan
        "current_plan_id": None, "completed_days": set(), "study_plan": None,
        # Doc intel
        "doc_intel": None, "doc_intel_map": {},
        # Flashcards
        ...
        "exam_start_time": None, "exam_duration": 30, "short_answers": {},
    }
    ...
    for edge in edges:
        ...
        if src in positions and tgt in positions:
            ...
            edge_traces.append(go.Scatter(
                ...
                mode="lines", line=dict(width=1.5, color="#e2e8f0"),
...
def show_main_app():
    ...
    with st.sidebar:
        ...
        mode = st.selectbox("Mode", [
            "💬 Study Chat", "🧪 Test Mode", "📅 Study Plan",
            "📈 Analytics", "🃏 Flashcards", "🗺️ Concept Map", "📝 Exam Mode"
        ])
        ...
        pdf_count = len(st.session_state.pdf_indexes)
        st.markdown(
            ...
            f'<div class="system-status">📚 Multi-PDF &nbsp;<b style="color:#f59e0b">{pdf_count}/{MAX_PDFS} loaded</b></div>',
            unsafe_allow_html=True
        )
 
        # Loaded PDFs list in sidebar
        if st.session_state.pdf_indexes:
            st.markdown("---")
            st.markdown('<div style="color:#f59e0b;font-size:11px;font-weight:700;letter-spacing:1px;margin-bottom:8px">LOADED PDFs</div>', unsafe_allow_html=True)
            for i, idx in enumerate(st.session_state.pdf_indexes):
                st.markdown(
                    f'<div class="pdf-card">'
                    f'<div class="pdf-name">📄 {idx["filename"]}</div>'
                    f'<div class="pdf-meta">{idx["page_count"]} pages · {idx["chunk_count"]} chunks</div>'
                    f'</div>', unsafe_allow_html=True
                )
                if st.button(f"Remove", key=f"remove_pdf_{i}"):
                    st.session_state.pdf_indexes.pop(i)
                    st.session_state.active_pdf_names.pop(i)
                    st.session_state.doc_intel_map = {}
        ...
        if st.button("Logout"):
            ...
            st.rerun()
        st.markdown('<div style="color:#475569;font-size:11px;text-align:center;margin-top:8px">GPT-4o-mini · FAISS · BM25 · Whisper</div>', unsafe_allow_html=True)
    ...
    remaining = MAX_PDFS - len(st.session_state.pdf_indexes)
    if remaining > 0:
        st.caption(f"Upload up to {remaining} more PDF(s) — max {MAX_PDF_SIZE_MB}MB each")
        ...
        st.warning(f"Maximum {MAX_PDFS} PDFs loaded. Remove a PDF from the sidebar to add more.")
    ...
    if uploaded_files:
        for uploaded_file in uploaded_files:
            ...
            if fname in st.session_state.active_pdf_names:
                continue
 
            # Check limit
            if len(st.session_state.pdf_indexes) >= MAX_PDFS:
                st.warning(f"Maximum {MAX_PDFS} PDFs reached.")
                break
 
            file_bytes = uploaded_file.read()
 
            # Validate size
            is_valid, err = validate_pdf(file_bytes, fname)
            if not is_valid:
                st.error(f"❌ {err}")
                continue
 
            with st.spinner(f"Processing {fname}..."):
                idx = build_pdf_index(file_bytes, fname)
                st.session_state.pdf_indexes.append(idx)
                st.session_state.active_pdf_names.append(fname)
                st.session_state.memory  = create_memory()
                st.session_state.chat    = []
                st.session_state.concept_map = None
                st.success(f"✅ {fname} — {idx['page_count']} pages, {idx['chunk_count']} chunks")
 
    # Need at least 1 PDF to continue
    if not st.session_state.pdf_indexes:
        st.markdown("### 👆 Upload a PDF to get started")
        c1, c2, c3, c4, c5, c6, c7 = st.columns(7)
        cards = [
            ("💬","Study Chat","Voice Q&A with structured answers."),
            ...
            ("🃏","Flashcards","Auto-generated spaced repetition."),
            ("🗺️","Concept Map","Visual network of topic connections."),
    ...
    # Use primary PDF index for single-PDF features
    primary   = st.session_state.pdf_indexes[0]
    db        = primary["db"]
    bm25_index = primary["bm25"]
    all_docs  = primary["all_docs"]
    split_docs = primary["split_docs"]
    adaptive_k = get_adaptive_k(user_id)
 
    multi_mode = len(st.session_state.pdf_indexes) > 1
 
    # ── DOCUMENT INTELLIGENCE ──────────────────────
    for idx in st.session_state.pdf_indexes:
        fname = idx["filename"]
        if fname not in st.session_state.doc_intel_map:
            with st.spinner(f"🔍 Analyzing {fname}..."):
                intel = analyze_document(idx["split_docs"])
                st.session_state.doc_intel_map[fname] = intel
 
    # Show intel for each PDF
    for fname, intel in st.session_state.doc_intel_map.items():
        ...
        st.markdown(
            ...
            f'<div class="doc-intel-title">📊 {fname}</div>'
            ...
            f'<span style="color:#64748b;font-size:12px;font-weight:700">💡 SUGGESTED: </span>'
    ...
    if multi_mode:
        st.info(f"🔍 **Multi-PDF mode active** — questions will search across all {len(st.session_state.pdf_indexes)} PDFs simultaneously")
    ...
    if mode == "💬 Study Chat":
        ...
        if st.button("Get Answer →", use_container_width=True):
            ...
            if not query.strip():
                ...
                with st.spinner("Searching across PDFs and generating answer..."):
 
                    if multi_mode:
                        # Multi-PDF retrieval
                        docs, confidence, source_map = multi_pdf_retrieve(
                            st.session_state.pdf_indexes, query
                        )
                        search_type = "multi-pdf"
                    else:
                        # Single PDF retrieval
                        docs, confidence, search_type = hybrid_retrieve(
                            db, bm25_index, all_docs, query, k=adaptive_k
                        )
                        pages      = get_sources(docs)
                        source_map = {primary["filename"]: pages}
        ...
        if st.session_state.chat:
            ...
            if last_ai:
                search_type_val = last_ai.get("search_type", "hybrid")
                if search_type_val == "multi-pdf":
                    search_badge = '<span class="badge badge-multi">📚 MULTI-PDF</span>'
                elif search_type_val == "hybrid":
                    search_badge = '<span class="badge badge-hybrid">⚡ HYBRID</span>'
                else:
                    search_badge = '<span class="badge badge-semantic">SEMANTIC</span>'
                ...
                with c1:
                    st.markdown(
                        f'<div class="metric-card"><div class="metric-val">{round(float(last_ai["confidence"]),1)}%</div>'
                        f'<div class="metric-label">Confidence {search_badge}</div></div>',
                        unsafe_allow_html=True)
                with c2:
                    source_map = last_ai.get("source_map", {})
                    if source_map:
                        sources_html = "".join(
                            f'<div class="source-card">'
                            f'<div class="source-pdf">📄 {pdf}</div>'
                            f'<div class="source-pages">Pages: {", ".join(str(p) for p in pages)}</div>'
                            f'</div>'
                            for pdf, pages in source_map.items()
                        )
                        st.markdown(
                            f'<div class="metric-card" style="text-align:left">'
                            f'<div class="metric-label" style="margin-bottom:8px">Sources</div>'
                            f'{sources_html}</div>',
                            unsafe_allow_html=True)
                    else:
                        st.markdown('<div class="metric-card"><div class="metric-val" style="font-size:14px">N/A</div><div class="metric-label">Sources</div></div>', unsafe_allow_html=True)
                with c3:
                    st.markdown(
                ...
                if st.session_state.flashcards:
                    st.info(f"🃏 {len(st.session_state.flashcards)} flashcards generated — go to **Flashcards** mode!")
    ...
    # ════════════════════════════════════════════
        ...
        with col1:
            topic_focus = st.text_input("Focus topic (optional)", placeholder="e.g. neural networks")
        with col2:
        ...
        if topic_focus.strip():
            ...
            st.markdown(
                f'<div style="margin-bottom:8px">Difficulty for <b>{topic_focus}</b>: '
                f'<span class="badge {diff_colors[final_diff]}">{final_diff}</span></div>',
                unsafe_allow_html=True)
        ...
        if st.button("Generate Quiz →", use_container_width=True):
            ...
            with st.spinner(f"Generating {final_diff} difficulty quiz..."):
                if multi_mode:
                    docs, _, _ = multi_pdf_retrieve(st.session_state.pdf_indexes, search_query)
                else:
        ...
        if st.session_state.quiz_data:
            ...
            with st.form("quiz_form"):
                ...
                if st.form_submit_button("Submit Answers →", use_container_width=True):
                    ...
                    save_quiz(user_id, topic_focus.strip() if topic_focus.strip() else "General",
            ...
            if st.session_state.quiz_submitted:
                ...
                st.markdown(
                    ...
                    f'<span style="color:#64748b;font-size:13px">📊 Next quiz difficulty: </span>'
    ...
    # ════════════════════════════════════════════
        ...
        if topics:
            st.info(f"Prioritizing your weak areas: **{', '.join(list(topics.keys())[:3])}**")
 
        if st.button("Generate New Study Plan →", use_container_width=True):
            with st.spinner("Building your personalized plan..."):
                all_split = []
                for idx in st.session_state.pdf_indexes:
                    all_split.extend(idx["split_docs"])
                plan = generate_study_plan(all_split, num_days=num_days, weak_topics=topics if topics else None)
                if plan:
    ...
    # ════════════════════════════════════════════
        ...
        if not has_data:
            ...
            if st.button("Download Session Report →", use_container_width=True):
                ...
                lines.append(f"  PDFs Loaded     : {len(st.session_state.pdf_indexes)}\n")
                if topics:
                ...
                lines.append("=" * 60)
                report_text = "\n".join(lines)
                st.download_button(label="Click to download", data=report_text,
    ...
    # ════════════════════════════════════════════
        ...
        st.caption("Visual network showing how concepts connect across your PDF(s).")
        ...
        if st.session_state.concept_map is None:
            if st.button("🗺️ Generate Concept Map", use_container_width=True):
                with st.spinner("Analyzing concepts..."):
                    ...
                    for idx in st.session_state.pdf_indexes:
                        all_split.extend(idx["split_docs"])
                    concept_data = generate_concept_map(all_split)
        ...
        if st.session_state.concept_map:
            ...
            st.markdown(
                ...
                '<span><span style="display:inline-block;width:12px;height:12px;background:#94a3b8;border-radius:50%;margin-right:6px"></span><span style="color:#64748b;font-size:12px">Detail</span></span>'
            ...
            if edges:
                ...
                for edge in edges:
                    src = node_label_map.get(edge.get("source"), edge.get("source",""))
                    tgt = node_label_map.get(edge.get("target"), edge.get("target",""))
                    rel = edge.get("relationship","relates to")
                    st.markdown(
                        f'<div style="padding:8px 14px;margin:4px 0;background:#ffffff;border:1px solid #e2e8f0;border-radius:8px;font-size:13px">'
                        f'<b style="color:#0a1628">{src}</b> <span style="color:#f59e0b;font-style:italic"> {rel} </span> <b style="color:#0a1628">{tgt}</b>'
                        f'</div>', unsafe_allow_html=True)
        else:
            st.info("Click 'Generate Concept Map' to visualize concept connections.")
    ...
    # ════════════════════════════════════════════
        ...
        st.caption("Timed mock exam with MCQs and short answers. Auto-submits when time runs out.")
        ...
        if st.session_state.exam_data is None:
            ...
            st.markdown(
                ...
                f'<span style="color:#334155">{num_mcq} MCQs + {num_short} Short Answers · {exam_duration} min · {exam_diff} · Total: {num_mcq+(num_short*5)} marks</span>'
                f'</div>', unsafe_allow_html=True)
 
            if st.button("🚀 Start Exam", use_container_width=True):
                with st.spinner("Generating your exam paper..."):
                    if multi_mode:
                        docs, _, _ = multi_pdf_retrieve(st.session_state.pdf_indexes, "all important topics")
                    else:
            ...
            st.markdown(
                ...
                f'<span class="badge {diff_colors.get(exam.get("difficulty","Medium"),"badge-medium")}">{exam.get("difficulty","Medium")}</span>'
                ...
                if short_qs:
                    ...
                    for q in short_qs:
                        ...
                        st.markdown(
                            ...
                            f'<div style="margin:10px 0"><b style="color:#64748b;font-size:12px">KEY POINTS:</b>{key_pts_html}</div>'
